...
/**
 * predeclaration of RecenterAction class. Used to make variadic templates from a passed tuple.
 * @tparam AxesConfig
...
class RecenterAction<AxesConfig, std::tuple<EventParameters...>> {
 public:
   ...
   */
  RecenterAction(std::string correction_name,
                 AxesConfig axes_configuration,
                 std::string base_q_name,
                 std::string previous_correction_name) : correction_name_(std::move(correction_name)),
   ...
   */
  [[nodiscard]] std::string GetName() const { return AddCorrectionStepToName(correction_name_); }
  ...
  bool LoadCorrectionFromFile(TDirectory *dir, TTreeReader &reader) {
    ...
    for (std::size_t i_harmonic = 0; i_harmonic < harmonics_vector_.size(); ++i_harmonic) {
      ...
      auto x = dynamic_cast<Qn::DataContainerStatistic *>(dir->Get((GetNameIncludingAxesAndHarmonics() + "/X_"
                                                                    + std::to_string(harmonic))
                                                                       .data()));
      auto y = dynamic_cast<Qn::DataContainerStatistic *>(dir->Get((GetNameIncludingAxesAndHarmonics() + "/Y_"
                                                                    + std::to_string(harmonic))
                                                                       .data()));
      if (!(x && y)) {
 ...
 private:
  ...
  void Initialize(TTreeReader &reader) {
    ...
    if (input_data.GetSetupStatus() < 0) {
      throw std::runtime_error("The Q-Vector entry "s + input_data.GetBranchName() + " in the tree is not valid. Cannot setup the recentering");
    }
  ...
  [[nodiscard]] std::string AddCorrectionStepToName(std::string correction_step) const {
    if (correction_step.empty()) return base_q_name_;
    else
...
auto Recentering(const std::string &correction_name,
                 ...
                 const std::string &base_q_name,
                 const std::string &previous_correction = "") {
  return RecenterAction<EventAxes, typename EventAxes::AxisValueTypeTuple>{correction_name, axes_configuration,
                                                                           base_q_name, previous_correction};
}
...
inline auto ApplyCorrectionsVector(DataFrame df, VectorOfCorrections corrections) {
  ...
  for (auto &result : corrections) {
    corrected_df = Qn::TemplateHelpers::DereferenceRResultPtr(result).ApplyCorrection(corrected_df);
  }
  return corrected_df;
}

...
/**
 * Template class to facilitate equal number of events per bin of the correction axes.
 * Currently only performs bin optimization in one dimension.
...
class EqualBinningHelper {
 public:
  using AxisTuple = typename AxesConfig::AxisTuple;/// Tuple containing the axes.
   ...
   */
  EqualBinningHelper(DataFrame df, const std::vector<std::string> &to_equalize) : dataframe_(df) {
 ...
 private:
  ...
  /**
   * Recursion.
   * Applies the equalization of number of entries algorithm on all specified axes.
   * Modifes the axes in axis_tuple.
  ...
  /**
   * Equalizes the bin of the specified axis reusing the axis borders and the number of bins.
  ...
  Qn::AxisD OptimizeBins(std::vector<double> data, const Qn::AxisD &axis) {
    ...
    auto position_low = std::distance(data.begin(), std::lower_bound(data.begin(), data.end(), low));
    auto position_high = std::distance(data.begin(), std::lower_bound(data.begin(), data.end(), high));
  ...
  /**
   * Calculates linearly spaced floating point numbers between two floating point values.
 ...
 * @param to_equalize vector of names of the axes which are supposed to be optimized

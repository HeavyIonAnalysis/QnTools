 ...
 * contributor license agreements.  See the NOTICES below
 ...
 * This version was modified by Lukas Kreis to allow storage in a ROOT file.
 */

#ifndef QN_TDIGEST_H_
#define QN_TDIGEST_H_
...
class TDigest {
  ...
  using Centroid = TDigestDetails::Centroid;
  using CentroidListQueue = TDigestDetails::CentroidListQueue;
  using CentroidComparator = TDigestDetails::CentroidComparator;
  using CentroidListComparator = TDigestDetails::CentroidListComparator;
  using CentroidList = TDigestDetails::CentroidList;
 ...
 public:
  ...
  TDigest(const TDigest &other) = default;
  TDigest& operator=(const TDigest &other) = default;
  ...
  Value cdfProcessed(Value x) const {
    ...
    if (processed_.size() == 0) { return 0.0;
